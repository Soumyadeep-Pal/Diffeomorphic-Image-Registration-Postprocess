...
def train(dir_path,
    ...
    ## Compute the Eigenvalues for dst
    grid = create_grid(D,H,W)
    eig_z = 2.0*torch.cos(math.pi*(grid[0,0,:,:,:])/(D-1))-2.0
    eig_y = 2.0*torch.cos(math.pi*(grid[0,1,:,:,:])/(H-1))-2.0
    eig_x = 2.0*torch.cos(math.pi*(grid[0,2,:,:,:])/(W-1))-2.0
    eig = eig_z + eig_y + eig_x
    eig = eig[1:D-1,1:H-1,1:W-1]
    
    
    ## Create dst matrices
    B_dst,D_dst,H_dst,W_dst = B,D-2,H-2,W-2
    dst_mat_D = create_dst_matrix(D_dst)
    dst_mat_H = create_dst_matrix(H_dst)
    dst_mat_W = create_dst_matrix(W_dst)
    ...
    for i in range(n_iter):
        ...
        _, flow_init = model(input_moving, input_fixed)
        flow,Jac_exp, Jac_final = plugin_dst(flow_init, dst_mat_D,dst_mat_H,dst_mat_W,eig)
        ...
        poisson_loss = losses.mse_loss(Jac_exp,Jac_final)
        
        loss = recon_loss + reg_param * grad_loss + poisson_param * poisson_loss
        
        save_metrics(loss, dir_path, "Training Loss")
        save_metrics(recon_loss, dir_path, "Ncc Loss")
        save_metrics(poisson_loss, dir_path, "Poisson Loss")                
...
if __name__ == "__main__":
    ...
    parser.add_argument("--poissonparam", type=float,dest="poisson_param",help="regularization parameter for minimizing loss of poisson reconstruction")
